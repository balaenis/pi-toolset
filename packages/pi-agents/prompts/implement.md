...
Execute as a chain. Name each step so later steps can reference earlier outputs via `{outputs.<name>}`. The worker's final output **must** include `## Completed`, `## Files Changed`, and `## Validation` (commands run + pass/fail, or `Not run: <reason>`).
