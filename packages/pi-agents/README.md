...
- **Background agents by default** - invocations return immediately and notify the parent via a custom message when they finish; set `runInBackground: false` to wait synchronously for the result. Hosts must not sleep or poll while waiting for a background completion
...
| `reviewer`      | Code review entry point (spawns `std-reviewer` + `spec-reviewer`) | `read, grep, find, ls, bash`           | spawns `std-reviewer` + `spec-reviewer` (`maxSubagentDepth: 1`) |
| `std-reviewer`  | Standards-axis diff review                                        | `read, grep, find, ls, bash`           | disabled (`maxSubagentDepth: 0`)                                |
| `spec-reviewer` | Spec-axis diff review                                             | `read, grep, find, ls, bash`           | disabled (`maxSubagentDepth: 0`)                                |
