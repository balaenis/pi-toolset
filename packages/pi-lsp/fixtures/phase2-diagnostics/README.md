...
diagnostics. It uses a real `typescript-language-server` binary from this fixture's
`node_modules/.bin`.
...
> LSP client can spawn `typescript-language-server`. If you launch pi without it
> (for example from a terminal where mise is not activated), the server cannot
> start and the `lsp` tool reports it is not ready.
...
You can watch the full lifecycle on stderr (the launch script sets
`PI_LSP_DEBUG=1`). Look for:
